...
class Excel_Report():
	...
	def read_xml(self):
		...
		s='::'
		suit_message = []
		self.log_name=self.log_addr+'\\'+self.report_time+'-log.log'
				...
				for j in test_case:
					...
					for i in kw:
						if i.getAttribute('name')=='Log':
							#第一个是得到的信息，是实际结果
							#接收错误AttributeError: 'NoneType' object has no attribute 'data'
							#在页面得到的是空值会出错
							try:
								...
								file.write(u'得到的实际值是空的'+s)#写入实际结果
						else:
							Log.append(u'未得到结果')
